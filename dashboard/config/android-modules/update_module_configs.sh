#!/bin/bash
KERNEL_DIR=$1
TARGET_CONFIG=$2

# Syz-kconv turns module configs ("=m") to yes ("=y"), but the module configs in 
# virtual_device.fragment are necessary when installing modules.
# This script finds all configs set to "=m" in virtual_device.fragment and uses
# ${KERNEL_DIR}'scripts/config to update configs generated by
# syz-kconf.
MODULE_CONFIGS=$(eval 'grep "=m$" dashboard/config/android-modules/virtual_device.fragment | sed -e "s/^/-m /" -e "s/=m//"')

# Update target config with virtual_device.fragment
${KERNEL_DIR}/scripts/config --file ${TARGET_CONFIG} ${MODULE_CONFIGS}
